/*function FlipSlider(options) {
  ...
  this.updatePagination = function () {
    ...
    for (var i = 0; i < slides.length; i++) {
      ...
      dot.dataset.index = i; // Stocăm indexul slide-ului în data attribute pentru a-l folosi ulterior
      pagination.appendChild(dot);
    }
    updateActiveDot(); // Actualizăm punctul activ
  };

  // Update active dot
  function updateActiveDot() {
    var activeIndex = Array.from(slides).indexOf(frontSlide);
    var dots = pagination.querySelectorAll(".dot");
    dots.forEach(function (dot, index) {
      if (index == activeIndex) {
        // Verificăm dacă indexul este egal cu indexul slide-ului activ
  ...
  this.goToSlide = function (index) {
    var currentIndex = Array.from(slides).indexOf(frontSlide);
